    ...
    public sealed partial class MainPage : Page
    {
        private void ForwardNotification(Notification notification)
        {
            ...
            foreach (var endPoint in Conf.CurrentConf.ApiEndPoints2)
            {
                ...
                Conf.Log($"[{notification.MessageId}] preparing to send data to {endPoint}...");
                ClientData.Send(endPoint, notification);
                processedEndPoints++;
            }

            Conf.Log($"[{notification.MessageId}] all uploads started, {processedEndPoints} endpoint(s) to go.", LogLevel.Complete);
        }
