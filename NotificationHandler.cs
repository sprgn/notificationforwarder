...
namespace Notification_Forwarder
{
    ...
    {
        IReadOnlyList<UserNotification> notifs = new List<UserNotification>();
        private async void NotificationHandler(object sender, UserNotificationChangedEventArgs e)
        {
            ...
            try
            {
                var notifsNew = await Listener.GetNotificationsAsync(NotificationKinds.Toast);
                lock (UnsentNotificationPool)
                {
                    lock (notifs)
                    {
                        ...
                        Conf.Log($"added to NewNotificationPool");
                        Notifications.AddRange(newlyAdded);
                        Conf.Log($"added to Notifications");
                        ...
                        Conf.Log($"notifications received #{newlyAdded.Count}");
                        notifs = new List<UserNotification>();
                    }
                }
            }
            catch (Exception ex)
